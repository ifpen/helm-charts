...
helm install filesender ifpen/filesender -f my-values.yaml
```
...
Pour la configuration complète, voir [ENTRA_ID_SETUP.md](ENTRA_ID_SETUP.md).
...
Déploiement :
```bash
helm install filesender ifpen/filesender -f values-minimal.yaml
```

### Exemple 2 : Configuration de production
...
Déploiement :
```bash
helm install filesender ifpen/filesender -f values-production.yaml
```
...
```bash
helm install filesender ifpen/filesender -f values-dev.yaml
...
helm diff upgrade filesender ifpen/filesender -f my-values.yaml

# Effectuer la mise à niveau
helm upgrade filesender ifpen/filesender -f my-values.yaml
```
...
- Issues GitHub : https://github.com/ifpen/helm-charts/issues
